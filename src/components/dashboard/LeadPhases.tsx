  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        let pipelineQuery = supabase.from('pipelines').select('id').eq('user_id', user.id);
        ...
        let {
          data: pipelineData,
          error: pipelineError
        } = await pipelineQuery.single();
        if (pipelineError || !pipelineData) {
          // Fallback to any pipeline this user has
          const {
            data: fallbackPipeline,
            error: fallbackError
          } = await supabase.from('pipelines').select('id').eq('user_id', user.id).limit(1).single();
        ...
        const {
          data: phasesData,
          error: phasesError
        } = await supabase.from('pipeline_phases').select(`
            id,
            name,
            order_index,
            leads:leads (
          ...
          `).eq('pipeline_id', pipelineData.id).order('order_index');
        if (phasesError) {

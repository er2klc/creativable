...
export const DashboardSidebar = () => {
    ...
    queryFn: async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('changelog_entries').select('version').order('version', {
          ascending: false
        }).limit(1).single();
  ...
  return <Sidebar className={`fixed group w-[72px] hover:w-[240px] transition-all no-scrollbar duration-300 ease-in-out ${isExpanded ? 'w-[240px]' : ''} z-[9999]`} {...handlers}>
